define([], function() {
    ...
    return function(options) {
        ...
        var applyFilter = function(filterCriterion, filterStrategy, collection){
            ...
            criterion = filterStrategy === "filter"? filterCriterion.trim() : filterCriterion;
            items = [] ;

            if(criterion){
                if(filterStrategy == "filter" ){
                    if( !filtered.filterFunction) throw( "Attempted to use 'filter' function, but none was defined" );
        ...
        original.on('add', function(models) {
            var coll = new original.constructor(), items;
            coll.add(models);
